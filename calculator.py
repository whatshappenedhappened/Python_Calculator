## 입출력 길이 제한 해재, 인덱스가 17이상이면 실수형태('ne+n') 형태로 출력하도록 변경 및 기타 예외처리 추가
## global 변수 static화
...
def input(val):
    ...
    if type(val) == str:                            ##### operator part
        ...
        if val == '.':                              ### decimal point part
        ...
        elif val == '+' or val == '-' or val == '*' or val == '/' or val == '%':             ################################## 오퍼레이터가 없으면 추가하고, 있으면 다음 인덱스의 값의 여부를 조회하여 있으면 합산값을 인덱스1에 저장후 오퍼레이터 오버라이딩
            ...
            if not input.calcValue[1] or input.calcValue[1] == '=':
                ...
                else: input.calcValue[0] = str(eval(total))

                input.calcValue[1] = val
                input.calcValue[2] = ''
            if input.calcValue[0] == 'inf': output_history.config(text=input.calcValue[:1])
            else: output_history.config(text=input.calcValue[:2])

        elif val == '=' or val == 'Enter':
            if input.calcValue[0] == 'inf':
                print("INFINITE NUMBER MY FRIEND")
                val = ''
            total = ''
            ...
            if input.calindex == 0 and input.calcValue[input.calindex] == '': input.calcValue[input.calindex] = '0'
            elif input.calindex == 2 and input.calcValue[2] == '': input.calcValue[input.calindex] = '0'
    ...
    if len(input.calcValue[input.calindex]) < 13:
        if input.optFin == 1 and (len(input.calcValue[0]) > 13 or len(input.calcValue[2]) > 13):
            ...
            output_current.config(height=1, font=('D2Coding', 30))
    elif len(input.calcValue[input.calindex]) > 13:
        output_current.config(height=2, font=('D2Coding', 17))
